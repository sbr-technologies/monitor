...
class AdditionalLanguageController extends CI_Controller {
    ...
    /**
     * Added for listing 
     * all the additional languages
     */
    public function index() {
        $data['title'] = 'Additional Language';
        $this->load->view('additional-language/index', $data);
    }

    /**
     * Added for listing all the 
     * additional languages areas through ajax
     */
    public function ajaxAdditionalLanguage() {
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->input->post('additional_language_name') !== "") {
                $search_array['additional_language_name'] = $this->input->post('additional_language_name');
            }
            ...
            $iTotalRecords = $this->additionalLanguageMaster_model->fetchAdditionalLanguageInfo('number', 0, $search_array);
            ...
            $result = $this->additionalLanguageMaster_model->fetchAdditionalLanguageList($search_array, $orderByData, $orderByType, $iDisplayStart, $iDisplayLength);
     ...
     * additional language
     */
    public function addAdditionalLanguage() {
        $data['title'] = 'Add New Additional Language';
        $config = array(
            array(
                'field' => 'name',
                'label' => 'name',
                'set_value' => 'name',
                'rules' => 'trim|required|callback_check_additional_language_name_exist_add',
                'errors' => array(
                    'required' => 'Please provide new additional language',
                    'check_additional_language_name_exist_add' => 'Oops !!! This language name is already taken',
                ),
            )
        );
        $this->form_validation->set_rules($config);
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('additional-language/addAdditionalLanguage', $data);
        } else {
            ...
            $inserted_id = $this->additionalLanguageMaster_model->add_language_master($inserting_data);
            redirect('additional-language');
        }
    }

    /**
     * Added for checking 
     * additional language is enlisted or 
     * not
     * @param string $name
     * @return boolean
     */
    public function check_additional_language_name_exist_add($name) {
        $result = $this->additionalLanguageMaster_model->get_additional_language_name($name);
        if ($result > 0) {
            return false;
        } else {
            return true;
     ...
     * @param string $name
     */
    public function ajax_additional_language_name_exist_add() {
        if ($this->input->is_ajax_request()) {
    ...
    /**
     * Added for deleting additional
     * language
     */
    public function deleteAdditionalLanguage() {
        if ($this->input->is_ajax_request()) {
            $additional_language_id = $this->input->post('additional_language_id');
            $main_additional_language_id = $this->encrypt_decrypt->decrypt($additional_language_id, ENCRYPT_DECRYPT_KEY);
            $update_array['status'] = "3";
            $this->additionalLanguageMaster_model->update_language_master($update_array, $main_additional_language_id);
            $records['status'] = 1;
            $this->output
                    ->set_content_type('application/json')
                    ->set_output(json_encode($records));
     ...
     * additional language
     */
    public function editAdditionalLanguage() {
        $data['title'] = 'Edit Additional Language';
        $encrypted_additional_language_id = $this->uri->segment(2);
        $main_additional_language_id = $this->encrypt_decrypt->decrypt($encrypted_additional_language_id, ENCRYPT_DECRYPT_KEY);
        $result = $this->additionalLanguageMaster_model->fetchAdditionalLanguageInfo('all', $main_additional_language_id, array());
        $data['result'] = $result[0];
        $data['encrypted_id'] = $encrypted_additional_language_id;
        $config = array(
            array(
                'field' => 'name',
                'label' => 'name',
                'set_value' => 'name',
                'rules' => 'trim|required|callback_additional_language_name_exist_edit',
                'errors' => array(
                    'required' => 'Please provide additional language name',
                    'additional_language_name_exist_edit' => 'Oops !!! Additional language name is already taken',
                ),
            )
        );
        $this->form_validation->set_rules($config);
        if ($this->form_validation->run() === FALSE) {
            $this->load->view('additional-language/editAdditionalLanguage', $data);
        } else {
            ...
            $this->additionalLanguageMaster_model->update_language_master($updating_data, $main_additional_language_id);
            redirect('additional-language');
        }
    }
     ...
     * @return boolean
     */
    public function additional_language_name_exist_edit() {
        $additional_language_id = $this->input->post('additional_language_id');
        $name = $this->input->post('name', true);
        $result = $this->additionalLanguageMaster_model->get_additional_language_details_name($name);
        if (count($result) === 0) {
            $response = true;
        } else {
            if ($result[0]['id'] === $additional_language_id) {
                $response = true;
            } else {
                $response = false;
            }
     ...
     * through ajax
     */
    public function additional_language_name_exist_edit_ajax() {
        if ($this->input->is_ajax_request()) {
            $additional_language_id = $this->input->post('additional_language_id');
            $name = $this->input->post('name', true);
            $result = $this->additionalLanguageMaster_model->get_additional_language_details_name($name);
